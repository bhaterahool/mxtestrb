...
import { SearchAdvanced, SearchResults, SearchResultItem } from '../components'
...
export const Search = () => {
  ...
  const [{ pelsrliteQueries }] = useRegistry()
  ...
  const onSearchSubmit = useCallback(() => {
    ...
    const queryParams = {
      ...
      ...(!searchtext && {
        ...
        ? `ticketid="${searchTerms}"` : `pelsearchterms="${getSuffix(
  ...
  const onTermsChange = useCallback(
    ...
    [searchParams.queryParams, session?.sessionId]
  )

  const onResetForm = () => {
    if (searchParams?.queryParams?.savedQuery) {
      searchParams.queryParams.savedQuery = ''
    }
    setSearchParams(params => ({ ...params, queryParams: '' }))
  }

  // Handler for changes to searchTerms - advanced search
  const onAdvancedTermsChange = useCallback(
    queryParams => {
      setSearchParams(params => ({ ...params, queryParams }))
    },
    [searchParams.queryParams]
  ...
  // Handler for setting current page.
  const onPageChange = page => setSearchParams(params => ({ ...params, page }))

  // Handler for search type toggles.
  ...
  // Hnadler for re-searching including CLOSED SRs (by removing the savedQuery)
        ...
        searchParams.queryParams.savedQuery !== '' && (
          <>
            <div className="pel--search-no-result">
              <ClickableTile handleClick={searchAgainIncludingClosedSR}>
